...
class ProbabilisticModel:
    ...
    def get_named_parameters(self) -> dict[str, nn.Parameter]:
        return dict(self.forward.named_parameters())
    ...
    def log_prob(
        ...
        #                   L(W) = log_prior(W) + log_likel(y | loc=NN(X))
        # where:
        ...
        log_likel = self.likelihood(self.forward(X)).log_prob(y).sum()
